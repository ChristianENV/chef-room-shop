...
export function MetricCard({ title, value, icon, change, subtitle, className }: MetricCardProps) {
  return (
          ...
          <div className="flex-1">
            ...
            {change && (
              <div className="mt-2 flex items-center gap-1">
                {change.type === 'increase' && <ArrowUpRight className="h-4 w-4 text-success" />}
                {change.type === 'decrease' && (
                  <ArrowDownRight className="h-4 w-4 text-destructive" />
                )}
                {change.type === 'neutral' && <Minus className="h-4 w-4 text-muted-foreground" />}
                <span
                  className={cn(
                ...
                <span className="font-serif text-sm text-muted-foreground">vs mes anterior</span>
              </div>
            )}
